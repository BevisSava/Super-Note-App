...
class Note {
    ...
    public function getAllByUser($user_id) {
        $query = "SELECT n.id, n.title, n.content, n.color, n.image_url, n.is_pinned, n.created_at, n.updated_at, n.password,
    ...
    public function create($user_id, $title, $content, $color, $image_url) {
       $query = "INSERT INTO " . $this->table . " 
                  (user_id, title, content, color, image_url) 
                  VALUES (:user_id, :title, :content, :color, :image_url)";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':user_id', $user_id);
    ...
    public function update($id, $user_id, $title, $content, $color, $image_url = null, $has_new_image = false) {
        if ($has_new_image) {
            $query = "UPDATE notes SET title = :title, content = :content, color = :color, image_url = :image_url WHERE id = :id AND user_id = :user_id";
        } else {
